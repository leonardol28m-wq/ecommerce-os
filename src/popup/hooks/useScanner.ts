...
import { createStoreFromScan } from '@/utils/helpers';
import { saveStore, getStoreByDomain, updateStore } from '@/db/database';

export function useScanner() {
  const { isScanning, setScanning, setScanResult, setCurrentUrl, addToast, addStore, updateStore: updateStoreInMemory } = useAppStore();
  ...
  const scanCurrentTab = useCallback(async () => {
    ...
    try {
      ...
      if (!tab?.id || !tab.url) { addToast({ type: 'error', message: 'No active tab found' }); return null; }
  ...
  const saveCurrentScan = useCallback(async (result: ScanResult) => {
    try {
      const { extractDomain } = await import('@/utils/helpers');
      const domain = extractDomain(result.url);
      const existing = await getStoreByDomain(domain);
      if (existing) {
        const updated = { ...existing, platform: result.platform, techStack: result.techStack, blueprint: result.blueprint, updatedAt: new Date().toISOString(), analysisCount: existing.analysisCount + 1 };
